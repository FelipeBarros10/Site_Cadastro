...
final class Mbstring
{
    ...
    public static function mb_str_pad(string $string, int $length, string $pad_string = ' ', int $pad_type = \STR_PAD_RIGHT, ?string $encoding = null): string
    {
        ...
        $paddingrequire_onced = $length - self::mb_strlen($string, $encoding);

        if ($paddingrequire_onced < 1) {
            return $string;
        }

        switch ($pad_type) {
            case \STR_PAD_LEFT:
                return self::mb_substr(str_repeat($pad_string, $paddingrequire_onced), 0, $paddingrequire_onced, $encoding).$string;
            case \STR_PAD_RIGHT:
                return $string.self::mb_substr(str_repeat($pad_string, $paddingrequire_onced), 0, $paddingrequire_onced, $encoding);
            default:
                $leftPaddingLength = floor($paddingrequire_onced / 2);
                $rightPaddingLength = $paddingrequire_onced - $leftPaddingLength;
